...
- **Regional new album releases:** selected as the eleventh finite M3 slice on 2026-08-26 after a current direct implementation, its live-gated pagination tests, and bounded anonymous two-page plus six-region structural probes. The finite route is region/offset-paged QQ new Albums → provider-neutral releases → existing Album/Track/queue flow. Heterogeneous Home shelves, editorial/tracking cards, notifications/cache, Album mutation or detail expansion, Track context navigation, and generic recommendation/catalog infrastructure remain out of scope. Implementation is active.
