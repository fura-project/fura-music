...
- `apps/flutter` contains adaptive Material 3 authentication, complete user-playlist collection, paged playlist-detail surfaces, short-lived Dart controllers/gateway adapters, one shared serialized platform-vault boundary, the selected foreground audio plugin edge, and Dart integration/widget tests.
...
The Bridge exposes this operation through one single-use `QqMusicMediaResolutionHandle`. Its input carries provider and opaque track identity; its success output contains only URI, format, quality, and validity, while its failure output is coarse and typed. The handle owns exact cancel/run/isActive lifecycle, drops the losing Provider/network future, and redacts both opaque identity and source URI from Rust diagnostics. The generated Dart DTO deliberately has no generated string representation. A product playback controller, queue, and Flutter affordance do not exist yet.

The Flutter playback edge now selects pinned `audioplayers` 6.8.1 for foreground single-track playback. It provides endorsed native plugins for every generated target without adding a separate playlist or background-service model. A packaged Linux integration proves a generated local MP3 can be loaded, played, paused, resumed, stopped, and disposed through GStreamer with exact temporary-file cleanup. This does not prove authenticated QQ URLs, cleartext-source policy, another target runtime, controller stale-event handling, or a product queue; no product playback controller or affordance exists yet.
