...
Authentication terminal presentation keeps error information separate from recovery actions. When saved-session verification or an active QR session asynchronously reaches a terminal failure, Flutter exposes only the existing title and explanation as one semantic live region; retry, cleanup, and replacement-code controls remain independent focusable actions. This is presentation behavior only and does not change credential classification, retry authority, or protocol state.
