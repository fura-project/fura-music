...
| Flutter and Rust stay in one process behind a thin typed boundary | Pass on Linux and Android x64; build-pass on Android ARM64 | Linux and Android packaged typed-FFI calls; actual Android signed-out application startup; both Android APKs contain the requested Rust bridge; source dependency review | Physical-device coverage remains a release-quality follow-up, not an M1 build blocker |
