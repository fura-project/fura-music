...
2. **Library sections still read as separate functional surfaces.** Observation: Playlists, favorite Albums, and favorite Artists retain correct independent state but use different headers, artwork scales, and density. Impact: personal music does not yet scan as one collection. Affected surfaces: Library root and its three sections. Bounded fix: unify section frame, collection item grammar, and desktop density while retaining controllers and routes. Risk: compact reachability, scroll/focus restoration, or unnecessary generic-controller extraction.
