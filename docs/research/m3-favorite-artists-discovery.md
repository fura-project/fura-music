...
The 2026-09-01 artwork follow-up cross-checked the pinned L-1124 and feeluown implementations: both derive a singer portrait from the safe Artist MID using QQ Music's `music/photo_new/T001...M000<mid>.jpg` image route rather than treating a response-specific avatar field as identity. A bounded public range request against a non-account fixture MID returned HTTP 206 with `image/jpeg`. The Provider now derives the HTTPS 300 px portrait only after MID validation and carries it as optional provider-neutral Artist artwork through Domain, typed Bridge, and Flutter. Missing or failed images remain truthful Material placeholders; no stored credential or account response was accessed.
