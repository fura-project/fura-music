...
Date: 2026-09-09. **HUMAN_EVIDENCE_REQUIRED for every real-account claim.** No real QR login, stored-account access, account write, cookie extraction or authenticated live call was performed by the Agent.
...
| Liked songs | weapi song/like/get, selected IDs through bounded Song detail | ≤1,000 unordered IDs; provider-owned `liked:<account>:<actual playlist ID>` route, with exact owner checks; raw cursor and omission counts; no recent-order claim |
