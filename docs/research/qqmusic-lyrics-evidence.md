...
- **Status:** Anonymous/authenticated Client and Provider mapping implemented; ordinary and literal-quote QRC paths live-proven; line-timed compatibility and optional-track isolation fixture-verified
...
4. [chenmozhijin/LDDC at `84631e8`](https://github.com/chenmozhijin/LDDC/tree/84631e8cd011fcc3f71ca0ae017e2c9758958ffc), especially its QRC parser/decryptor. This GPL-3.0-only source is evidence only and must not be copied into the project. Its parser independently agrees on line and token timing semantics and treats `Lyric_1` as a QQ-specific envelope whose content ends at the element's closing `"/>`, rather than requiring lyric text itself to remain a valid XML attribute.
...
The decoded cloud value is an XML-shaped document whose relevant payload is the `LyricContent` attribute of a `Lyric_1` element with lyric type `1`. XML entities must be decoded before parsing the embedded QRC text. Strict XML remains the primary parser, and a regular expression alone is not sufficient as the production trust boundary because lyric text can contain escaped entities, ampersands, angle brackets, and literal quotes.
...
QQ's decrypted cloud-QRC envelope is XML-shaped but its `LyricContent` value is not guaranteed to use strict XML entity escaping. An earlier content-free failure established the need to preserve literal/unknown ampersands in this one field. On 2026-09-12, bounded anonymous probes for two exact public Tracks independently reached `quick-xml`'s `ExpectedEq` attribute error after successful request and decryption. Content-free structural counts then proved two and four literal double quotes respectively inside `LyricContent`; neither document contained a raw angle bracket or ampersand in that payload. Those quotes prematurely close a normal XML attribute and make subsequent lyric text look like another attribute.

Fura therefore keeps the strict XML reader as the primary path and recovers only this evidenced QQ pseudo-XML shape. The recovery requires the exact single-track `<Lyric_1 LyricType="1" LyricContent="` marker, at least one literal inner quote, the expected `LyricInfo`/`QrcInfos` closing hierarchy, and a fully valid XML skeleton after removing only the content payload. The recovered field still passes through the same bounded entity decoder and strict QRC timing parser. Wrong lyric types, arbitrary malformed XML, duplicate envelopes, trailing markup, bad timing and existing size/count overflows remain rejected. This is deliberately narrower than accepting arbitrary broken XML or falling back to plain text.
...
After implementation, the opt-in Rust `live_lyrics` test ran the same public MID through the actual bounded client request, QQ-compatible decryptor, XML reader, and QRC parser. It confirmed a nonempty original line set with at least one timed segment without printing or retaining ciphertext or lyric text. On 2026-09-01 the gate passed again with no Cookie and only zero/empty anonymous account fields; the Provider now uses this path while signed out instead of rejecting before transport. On 2026-09-03 the same one-request anonymous gate passed after the compatibility change.

On 2026-09-12 a second ignored live regression searched for the exact public title, artist and duration of both reported literal-quote Tracks, then passed their provider-returned MID/type directly to the production lyric method. The four-request probe completed anonymously and parsed 68 and 25 original lines respectively. It printed only parser stages, quote/reserved-character counts and line counts; it retained no search result body, identity, ciphertext or lyric text. This proves the implemented anonymous request/decode/pseudo-XML/timing path for those exact catalog entries on that date. It still does not prove authenticated-only outcomes or visible playback synchronization in a packaged application.
...
5. A maintainer rerun of the two reported literal-quote Tracks in the rebuilt application. Their exact anonymous Core paths now pass; this Human check must confirm the Bridge/controller refresh and visible synchronized rendering rather than re-prove the protocol parser.
