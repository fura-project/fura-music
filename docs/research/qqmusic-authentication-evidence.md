...
The current QQ `xlogin` page reported build `202609011612` and loaded `https://qq-web.cdn-go.cn/monorepo/c1db8078/ptlogin/js/c_login_2.js`. Static inspection found that HTTPS quick login probes odd loopback ports `4301`, `4303`, `4305`, `4307`, and `4309`; calls `pt_get_uins` with `pt_local_tk`, QQ login app `716027609`, DAID `383`, QQ Music Connect app `100497308`, and `login_jump`; then calls `pt_get_st` for the explicitly selected account. The local-login script's `str.hash33(clientkey)` starts from zero. That token is sent to `https://ssl.ptlogin2.qq.com/jump`, whose current successful callback points to a signed `https://ssl.ptlogin2.graph.qq.com/check_sig` URL. Following that URL establishes the QQ Connect `p_skey` and redirects through `login_jump`; the existing QQ Connect authorization and QQ Music credential exchange then apply. The 5381 seed belongs to the later QQ Connect `g_tk` calculation and is not interchangeable with `pt_local_tk`. The same script fetches account images through `ssl.ptlogin2.qq.com/getface`.

The inspected Linux environment had the current QQ client listening on `127.0.0.1:4301`. A connection-only TLS check found a publicly trusted Tencent certificate with `localhost.ptlogin2.qq.com` in its SAN. With the maintainer's explicit authorization on 2026-09-11, a disabled-by-default live check discovered exactly one local account and completed `pt_get_st`, `jump`, `check_sig`, QQ Connect authorization, and QQ Music credential exchange. The check asserted only the account count, QQ login type, and presence of the final credential fields. It printed and persisted no account identity, nickname, local token/ticket, cookie, signed URL, OAuth code, credential, or response body.

The production candidate pins the local hostname to `127.0.0.1`, bounds all loopback responses and timeouts, accepts at most ten structurally valid accounts, and retains the raw QQ identifier plus local token/ticket only inside an opaque Rust session. Flutter receives a nickname, masked account hint, and attempt-local selection index. A first attempt also performed the script's `getface` lookup serially before returning any account; the maintainer's five-second failure report exposed that as an incorrect interaction gate. The corrected candidate returns choices immediately after staggered concurrent probes and uses a generic account icon. It parses both the current fixed JavaScript assignments and legacy direct callback forms without evaluating JavaScript, computes `pt_local_tk` with the evidenced zero seed, and follows only a strictly validated `check_sig` target matching the selected account and expected QQ/Connect application identities. Real avatar loading remains deferred until it can be lazy/non-blocking without exposing raw QQ identity. Complete request/exchange behavior is fixture-tested; discovery-only and sole-account authorization live checks are separately gated. Core authorization now has one successful Linux real-account observation, while visible Fura dialog completion, secure-vault restore after a clean restart, QR fallback regression, and Windows/macOS compatibility remain Human evidence.
