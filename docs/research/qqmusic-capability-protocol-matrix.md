...
| Recent Tracks read | musicu recent history | `music.musichallSong.RecentPlayList/GetRecentPlayList` | signed authenticated web envelope; `uin/begin/num` | Authentication required | Request/signature/mapping fixtures pass; real-account order and QQ/WeChat parity pending Human review | Current credential, Cookie, UIN, key-derived `g_tk`, `zzc` body signature | offset/max 100; absent total represented only as safe lower bound until a short page; invalid raw rows still advance | Explicit rejection clears only the current generation; empty is valid; unknown nonzero codes are service failure | `wangwalk/qqm` source plus maintainer evidence and Fura fixtures; 2026-09-08 | `PRIMARY`, Human-gated live acceptance |
| Recent Tracks write | Tencent IoT/H5 only | `reportRecentPlay({ResourceId, Type})` in Tencent IoT SDK | different device/H5 authorization environment; no ordinary musicu request exposed | Not applicable | No ordinary QQ-session protocol or real-account verification | Unknown for Fura credential | Unknown identity, acknowledgement, dedupe and lifecycle timing | Must not infer a private method or playback threshold | Tencent IoT docs plus negative bounded public-source survey; 2026-09-08 | `BLOCKED`; `RecentHistoryWrite` not advertised |
...
| Lyrics | musicu PlayLyricInfo | `music.musichallSong.PlayLyricInfo/GetPlayLyricInfo` | `ct=11`, `cv=13020508`; encrypted QRC or line-timed original plus optional translation/romanization | Ordinary and exact two-Track literal-quote ignored live QRC passes; no Cookie or fabricated identity | Fixture pass; M1 visible playback synchronization remains pending Human observation | Optional single-owner credential | One Track; bounded ciphertext, decompressed text, lines and timed segments; line-only results have no fabricated word timing; strict XML is primary, while an exact single-track QQ envelope may recover evidenced literal quotes only when its content-free XML skeleton remains valid | Empty/no usable original is `Unavailable`, not auth; explicit rejection STOP; malformed required original/envelope is invalid; malformed optional tracks are omitted without discarding a valid original | Fura source/fixture/live; independent route/parser corroboration and content-free structural diagnostics; 2026-09-12 | `PRIMARY`; ordinary and literal-quote QRC live-proven, line-timed/optional-isolation paths fixture-proven |
...
| Media VKey, guest | musicu VKey | `music.vkey.GetVkey/UrlGetVkey` | `ct=11`, `cv=13020508`; anonymous `uin=0`, no Cookie | Fixture plus ignored 24-Track/207-candidate comparison pass | Not applicable | None | M500 MP3, then C200 low M4A only after typed `Unavailable`; exact filename/path; short-lived TTL | Per-Track unavailable remains truthful and does not infer VIP/region/copyright; malformed identity/path STOP | Fura source/fixture/live; route/filename/format comparison; 2026-09-07 | `PRIMARY` |
| Media VKey, signed in | primary `music.vkey.GetVkey/UrlGetVkey`; exact-`104003` compatibility `vkey.GetVkeyServer/CgiGetVkey` | current mobile-style route first, desktop-compatible route second for the same profile only | primary `ct=11`, `cv=13020508`; compatibility `ct=19`, `cv=0`, authenticated `uin/authst`, Cookie, `loginflag=1`, `platform=20` | Guest route and 207-candidate comparison prove both response families but do not trigger production endpoint rotation | Fixture pass; maintainer reports ordinary playback success and one official-client-playable Track rejected by the primary route; exact compatibility result pending retest | Current credential from sole session owner | Lossless tries F000 FLAC, M800 MP3, M500 MP3, then C200 M4A; High omits F000; Standard starts at M500; exact primary `104003` tries the same profile once through compatibility before lowering quality; returned format/quality is actual | Only primary per-item `104003` selects the compatibility route; every response rechecks session ownership; compatibility unavailable may lower quality, while credential, replacement, rate/risk, network, service and malformed outcomes STOP | Fura source/fixtures, independent active source, anonymous exact-Track comparison, coarse Human playback plus official-client comparison; 2026-09-07 | Conditional compatibility `PRIMARY`; exact real-account result pending Human review |
...
| Official Playlists | musicu PlayListPlazaServer | `playlist.PlayListPlazaServer/get_playlist_by_category` | anonymous GET `data`; `ct=24`; `id=titleid=3317`, `curPage/size`, `order=5` | Anonymous category and two bounded list-page probes return global/method code 0; category service directly maps 3317 to 官方歌单 | Not needed | None | one-based provider page, max 30; `total/v_playlist`; identity `tid`; adjacent pages may overlap and are deduped above Client | Missing total/list, non-advancing page, oversized response or invalid required identity/title are shape failures; creator/play count/tags remain optional source fields | Fura source/fixture/live; jsososo request/category evidence and independent yakult service-family corroboration; 2026-09-19 | `PRIMARY`; Home production candidate, Human runtime/visual review pending |
...
| Media VKey | EVkey / extended-format requests | musicu media authorization | F000/O800/O600/O400/C600/C400/C200 candidates | 24-Track anonymous comparison and bounded byte probes | F000 request shape is independently corroborated; authenticated result pending Human selection | Production uses C200 low M4A as final unavailable fallback and exposes F000 FLAC only for explicit SQ preference | source survey plus Fura live comparison; 2026-09-07 | C200 `PRIMARY` fallback; F000 Human-gated candidate; Ogg/C600/C400 not promoted because they added no safer unique coverage |
