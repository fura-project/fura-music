...
- QR lifecycle cancellation is generation-based in `WechatQrLoginCoordinator`: replace, cancel, session drop, and coordinator drop wake the lifecycle watcher and drop the losing HTTP future. Always recheck the generation after an await; otherwise a late 405 can authenticate a superseded UI session. Overall deadline/retry policy is still pending.
