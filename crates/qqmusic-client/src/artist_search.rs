    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::RateLimited {
                ...
                formatter,
                "Artist search was rate limited with global code {global_code} and result code {result_code:?}"
            ),
...
fn map_response<E>(
    ...
    match classify_musicu_codes(global_code, result_code) {
        ...
        Err(QqProtocolOutcome::RateLimited) => {
            return Err(QqMusicArtistSearchError::RateLimited {
                global_code,
                result_code,
            });
        }
...
mod tests {
    ...
    async fn classifies_rate_limit_without_fabricating_an_empty_page() {
        ...
        assert!(matches!(
            client.search_artists("query", 1, 5).await,
            Err(QqMusicArtistSearchError::RateLimited {
                global_code: 0,
