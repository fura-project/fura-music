...
`fura music` is a modern, open-source, cross-platform, QQ Music-first client built with Flutter and an in-process Rust core. The repository and internal package names remain `flutterustmusic` for compatibility.

The first in-process vertical slice implements sign-in, credential restore, user playlists and details, foreground playback and queue controls, and synchronized word-level lyrics. M3 adds direct QQ Music Track, Artist, Album, and Playlist search, Album and Artist browsing, canonical Album metadata with credited-Artist navigation, Artist discography navigation, public recommended-playlist discovery, grouped current QQ Music rankings, authenticated Radar Track recommendations, regional new-Album releases, typed new-song channels, authenticated favorite Album and Artist collections, playlist Track-to-Album/Artist navigation, current-Track Album/Artist navigation, and an adaptive immersive artwork-and-lyrics now-playing page. M4 consolidates those journeys into a deliberate official-Flutter Material 3 desktop/compact baseline. M5 checkpoints a Home-first mainstream QQ Music product with distinct retained Discover, Search, Library, and Now Playing responsibilities, authoritative playback modes, bounded read-only comments, and one Track-associated MV journey. One user-operated real-account playback/queue/lyric observation remains before the M1 end-to-end acceptance claim.

## Status

The project has a working M1 implementation with offline regression coverage and bounded Linux/Android runtime evidence. M2 through M6 and the first-release Core-capability workstream are checkpointed. M7 added bounded QQ Music-familiar shell, Home, Library, and persistent-player presentation work but is not checkpointed; page-level visual work remains paused for maintainer review of the current Home before continuing one page at a time. This is not release readiness, broad live catalog compatibility, remote mutation/MV playback validation, or M1 acceptance; real QQ Music playback remains subject to the user-operated observation above. Product boundaries and architectural decisions are recorded in [PROJECT.md](PROJECT.md), [ARCHITECTURE.md](ARCHITECTURE.md), and [ROADMAP.md](ROADMAP.md). Current execution state lives in [PROGRESS.md](PROGRESS.md), with exact capability and blocker evidence in [the first-release audit](docs/development/first-release-capability-audit.md) and [checkpoint review](docs/development/first-release-core-capability-checkpoint.md).
...
Playback uses the endorsed Linux `audioplayers` implementation and therefore also needs GStreamer 1.0 core, app, and audio development modules plus runtime plugins for the formats being played. Local packaged integrations currently prove MP3, AAC-in-M4A, and FLAC decode on the recorded development environment; installing headers alone does not prove runtime codec availability on another distribution.
Track-associated MV uses the separately pinned `media_kit` video stack behind a project-owned disposable adapter; it does not replace the foreground music engine or Rust Queue. A packaged Linux integration proves a generated local H.264 MP4 can decode and follow pause/seek/resume, while Linux Release and Android x64 Debug packaging pass. Those checks do not prove live QQ MV source playback, hardware decode, physical devices, or Apple/Windows runtime behavior. Before any external distribution, the bundled native media libraries need a per-artifact license/notice review as tracked by TD-006.
