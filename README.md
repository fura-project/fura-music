...
The project has a working M1 implementation with offline regression coverage and bounded Linux/Android runtime evidence. M2 through M6 are checkpointed. M7 added bounded QQ Music-familiar shell, Home, Library, and persistent-player presentation work but is not checkpointed. After a completed complexity-paydown pass, page-level visual work is now paused while a first-release core-capability audit closes or explicitly blocks the remaining Account, Home-data, Library-mutation, Playback/media, Settings, and platform foundations. This is not release readiness, broad live catalog compatibility, remote MV playback validation, or M1 acceptance; real QQ Music playback remains subject to the user-operated observation above. Product boundaries and architectural decisions are recorded in [PROJECT.md](PROJECT.md), [ARCHITECTURE.md](ARCHITECTURE.md), and [ROADMAP.md](ROADMAP.md). Current execution state lives in [PROGRESS.md](PROGRESS.md), and the capability matrix is in [the first-release audit](docs/development/first-release-capability-audit.md).
