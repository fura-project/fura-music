...
- HD-015 authorizes a manual GitHub Actions packaging gate for maintainer runtime testing. Run 33407983713 passed the offline quality gate and Android, Windows, macOS, and iOS Simulator packages. Its Android job also passed the new Release-manifest Internet-permission and ARM64 Rust-payload assertions, while its pinned `windows-2022` Release build passed. Linux alone still failed during dependency installation after all three bounded retries, disproving a one-off apt transport failure. Because GitHub has announced `ubuntu-22.04` deprecation beginning 2026-09-17, the Linux job now targets `ubuntu-24.04`, consumes its documented preinstalled compiler/build/D-Bus/Xvfb tools, and installs only the missing GTK, libsecret, and GStreamer development/runtime libraries. A new remote run and physical Android retest remain required; these local changes do not establish either result.
